 ...
 *    (例:采样周期 50µs、脉冲 >1% 宽度 400µs → 输出 8 行)。空闲(≤1%)不输出,
 *    不占用串口带宽。
...
#define FW_VERSION "ver3.07.00"   // 固件版本(每次改动由 Claude 递增)
...
constexpr uint32_t ADC_SAMPLE_US      = 100;     // 采样周期 100µs = 10kHz(建议值,见回复里的说明;够看清 ~1ms 级脉冲,
                                                  // 用 esp_timer 硬件定时器采样,不占用/不受 loop() 抖动影响)
